...
func TestUpdateDeploymentInput_AgwMarshaling(t *testing.T) {
	tests := []struct {
		...
		{
			name: "a customer-run gateway sends its hostname",
			input: UpdateDeploymentInput{
				Agw: NewAgwUpdate(&AgwPatch{Hostname: strPtr("gw.example.com")}),
			},
			contains: `"agw":{"hostname":"gw.example.com","idp_id":null}`,
		},
		{
			// Clearing the selection has to travel as null. Omitted reads as
			// "unchanged" to the API, so the stored id would outlive the
			// configuration that no longer names one.
			name: "a cleared idp travels as null",
			input: UpdateDeploymentInput{
				Agw: NewAgwUpdate(&AgwPatch{IdpID: nil}),
			},
			contains: `"agw":{"idp_id":null}`,
		},
		{
			// A hosted gateway carries no hostname, and the API refuses the
			// key outright for that kind, even holding null.
			name: "a hosted gateway sends the idp alone",
			input: UpdateDeploymentInput{
				Agw: NewAgwUpdate(&AgwPatch{IdpID: strPtr("idp-1")}),
			},
			contains: `"agw":{"idp_id":"idp-1"}`,
		},
	}
