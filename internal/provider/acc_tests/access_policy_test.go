...
func TestAccResourceAccessPolicy_withAzureWifDeliveryServiceAccount(t *testing.T) {
	resource.ParallelTest(t, resource.TestCase{
		ProviderFactories: providerFactories,
		CheckDestroy:      validateResourceDestroyed("access_policy", "v1/access_policies"),
		Steps: []resource.TestStep{
			{
				Config: accessPolicyAzureWifDeliveryServiceAccountStep(),
				Check: resource.ComposeTestCheckFunc(
					resource.TestMatchResourceAttr(
						"hush_access_policy.test", "id", regexp.MustCompile("^apl-.+$"),
					),
					...
					resource.TestCheckResourceAttr(
						"hush_access_policy.test", "azure_wif_delivery_config.0.subject_kind", "service_account",
					),
				),
			},
		},
...
func accessPolicyAzureWifDeliveryServiceAccountStep() string {
	return `
resource "hush_azure_wif_access_credential" "test" {
  name           = "test-azure-wif-cred-sa"
  description    = "Azure WIF credential for service account test"
  deployment_ids = ["` + mockDeploymentID + `"]
}

resource "hush_access_policy" "test" {
  name                 = "test-policy-azure-wif-sa"
  description          = "Azure WIF delivery with service account"
  enabled              = true
  access_credential_id = hush_azure_wif_access_credential.test.id
  deployment_ids       = ["` + mockDeploymentID + `"]
