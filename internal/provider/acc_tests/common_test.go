...
func validateResourceDestroyed(resource, resourcePath string) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		...
		for _, rs := range s.RootModule().Resources {
			...
			case "azure_wif_access_credential":
				_, err = client.GetAzureWifAccessCredential(context.Background(), c, resourceId)
			...
			case "auth0_access_credential":
				_, err = client.GetAuth0AccessCredential(context.Background(), c, resourceId)
			case "auth0_access_privilege":
				_, err = client.GetAuth0AccessPrivilege(context.Background(), c, resourceId)
			default:
