...
func init() {
	registerMockSetup(func(ms *testutil.MockServer) {
		// shepherd resolves an agw change with model_copy(update=changes), so
		// a patch naming one member leaves the others standing. Without this
		// the mock replaces the object, and patching only idp_id would drop
		// the region a hosted gateway was placed in.
		ms.MergeNested("deployment", "agw")
		...
		// The API resolves agw.idp_id against the org's enabled providers and
		// refuses an unknown one. Without this the mock takes any string, and
		// nothing would notice the provider sending the wrong thing.
		refuseUnknownIdp := func(
			op testutil.Operation, obj map[string]any,
		) *testutil.HookError {
			...
			idp, present := agw["idp_id"].(string)
			// A null clears the selection and is never looked up. Anything
			// else is, including the empty string, which resolves to no
			// provider -- so sending "" in place of null is a 422, not a
			// quieter way of clearing.
			if !present || idp == agwIdpID || idp == agwIdpID2 {
				return nil
			}
			return &testutil.HookError{Status: 422, Detail: "unknown or disabled idp"}
		}
		ms.OnOperation("deployment", testutil.OpCreate, refuseUnknownIdp)
		ms.OnOperation("deployment", testutil.OpUpdate, refuseUnknownIdp)
...
// TestAccResourceDeploymentAgwIdp walks the gateway's identity provider through
// selection, a change and removal, on both kinds of gateway -- it is the one
// member of the block that applies to each and that a patch may carry.
func TestAccResourceDeploymentAgwIdp(t *testing.T) {
	resource.ParallelTest(t, resource.TestCase{
		ProviderFactories: providerFactories,
		CheckDestroy:      validateResourceDestroyed("deployment", "v1/deployments"),
		Steps: []resource.TestStep{
			{
				Config: deploymentAgwIdpConfig("k8s", agwIdpID),
				Check: resource.TestCheckResourceAttr("hush_deployment.idp",
					"agw.0.idp_id", agwIdpID),
			},
			{
				Config: deploymentAgwIdpConfig("k8s", agwIdpID2),
				Check: resource.TestCheckResourceAttr("hush_deployment.idp",
					"agw.0.idp_id", agwIdpID2),
			},
			{
				// Dropping it has to reach the API as null. Omitted reads as
				// "unchanged", which would leave the stored selection behind a
				// configuration that no longer names one -- a plan that never
				// settles.
				Config: deploymentAgwIdpConfig("k8s", ""),
				Check: resource.TestCheckResourceAttr("hush_deployment.idp",
					"agw.0.idp_id", ""),
			},
		},
...
// The selection is patchable on a gateway Hush runs too, which is the only
// thing about a hosted agw block that an update may touch.
func TestAccResourceDeploymentHostedAgwIdp(t *testing.T) {
	resource.ParallelTest(t, resource.TestCase{
		ProviderFactories: providerFactories,
		CheckDestroy:      validateResourceDestroyed("deployment", "v1/deployments"),
		Steps: []resource.TestStep{
			{
				Config: deploymentHostedAgwIdpConfig(agwIdpID),
				Check: resource.TestCheckResourceAttr("hush_deployment.hosted_idp",
					"agw.0.idp_id", agwIdpID),
			},
			{
				Config: deploymentHostedAgwIdpConfig(agwIdpID2),
				Check: resource.TestCheckResourceAttr("hush_deployment.hosted_idp",
					"agw.0.idp_id", agwIdpID2),
			},
			{
				Config: deploymentHostedAgwIdpConfig(""),
				Check: resource.TestCheckResourceAttr("hush_deployment.hosted_idp",
					"agw.0.idp_id", ""),
			},
		},
...
func deploymentAgwIdpConfig(kind, idpID string) string {
	idp := ""
	if idpID != "" {
		idp = fmt.Sprintf("\n    idp_id = %q", idpID)
	}
	return fmt.Sprintf(`
resource "hush_deployment" "idp" {
  name = "tf-acc-agw-idp"
  kind = %q
...
`, kind, agwHostname, idp)
}

func deploymentHostedAgwIdpConfig(idpID string) string {
	idp := ""
	if idpID != "" {
		idp = fmt.Sprintf("\n    idp_id = %q", idpID)
	}
	return fmt.Sprintf(`
resource "hush_deployment" "hosted_idp" {
  name = "tf-acc-agw-hosted-idp"
  kind = %q
...
`, deploymentKindHosted, agwRegion, idp)
}
