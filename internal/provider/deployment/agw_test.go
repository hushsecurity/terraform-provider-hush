...
// An update carries only what a patch may change. A region is fixed when the
// gateway is placed and has no field on the update model, so it never travels.
func TestExpandAgwUpdateOnACustomerRunGateway(t *testing.T) {
	d := resourceDataFor(t, map[string]any{
		"kind": "k8s",
		"agw": []any{map[string]any{
			"hostname": "gw.example.com", "region": "fra", "idp_id": "idp-1",
		}},
	})

	got := expandAgwUpdate(d)
	if got == nil {
		t.Fatal("expected an agw patch, got nil")
	}
	if got.Hostname == nil || *got.Hostname != "gw.example.com" {
		t.Fatalf("unexpected hostname %v", got.Hostname)
	}
	if got.IdpID == nil || *got.IdpID != "idp-1" {
		t.Fatalf("unexpected idp_id %v", got.IdpID)
	}
}

// A hosted gateway is patched for its idp alone. The API refuses a hostname on
// that kind outright, so the key must not be sent even holding null.
func TestExpandAgwUpdateOnAHostedGateway(t *testing.T) {
	d := resourceDataFor(t, map[string]any{
		"kind": deploymentKindHosted,
		"agw":  []any{map[string]any{"region": "fra", "idp_id": "idp-1"}},
	})
	...
	if got.Hostname != nil {
		t.Fatalf("a hosted gateway must send no hostname, got %v", *got.Hostname)
	}
	if got.IdpID == nil || *got.IdpID != "idp-1" {
		t.Fatalf("unexpected idp_id %v", got.IdpID)
	}
...
// Dropping idp_id from the configuration must reach the API as null, or the
// stored selection outlives the configuration that no longer names one.
func TestExpandAgwUpdateClearsTheIdp(t *testing.T) {
	d := resourceDataFor(t, map[string]any{
		"kind": "k8s",
		"agw":  []any{map[string]any{"hostname": "gw.example.com"}},
	})
	...
	if got.IdpID != nil {
		t.Fatalf("expected a nil idp_id, which marshals to null, got %v", *got.IdpID)
	}
}
...
func TestRegionRefusalNamesAPathThatWorks(t *testing.T) {
	err := hostedDiff(t, "fra", "iad")
	if err == nil {
		t.Fatal("expected the move to be refused")
	}
...
func planFor(t *testing.T, kind string, agw any, extra map[string]any) error {
	t.Helper()
	raw := map[string]any{"name": "d", "kind": kind}
	switch block := agw.(type) {
	case map[string]any:
		raw["agw"] = []any{block}
	case string:
		// The whole block list is unknown, not one member of it.
		raw["agw"] = block
	}
	for k, v := range extra {
		raw[k] = v
...
func TestAgwKindRules(t *testing.T) {
	tests := []struct {
		name string
		kind string
		// A map is one block; the unknown sentinel is a block list Terraform
		// cannot resolve until the apply.
		agw     any
		...
		// A block whose presence itself is unknown cannot be judged either: a
		// dynamic block over an unknown for_each reads back as no block, and
		// refusing it would refuse a configuration that may well carry one.
		{name: "hosted with an unknown block", kind: deploymentKindHosted,
			agw: unknownValue},
