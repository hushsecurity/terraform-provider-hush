...
const (
	...
	secretAccessKeyWODesc  = "The AWS secret access key (write-only). This is a write-only attribute that is more secure than `secret_access_key` because Terraform will not store this value in the state file. Only valid when `engine` is `elasticache`. Must be set together with `access_key_id`; omit both to use AWS workload identity federation."
	...
	clientSecretWODesc     = "The client secret of the Azure application identified by `client_id` (write-only). This is a write-only attribute that is more secure than `client_secret` because Terraform will not store this value in the state file. Only valid when `engine` is `azure_managed_redis`. Must be set together with `client_id`."
